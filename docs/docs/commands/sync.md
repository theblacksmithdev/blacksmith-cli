...
For data fetching, prefer the resource API hooks in `src/api/hooks/` (generated by `make:resource`) over the raw auto-generated hooks:

```typescript
// Preferred — resource API hooks with pagination, search, and cache invalidation
import { useProducts, useCreateProduct } from '@/api/hooks/products';

// Lower-level — auto-generated query options (use when you need direct access)
