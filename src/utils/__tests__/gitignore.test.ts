...
describe('ensureGitignore', () => {
  ...
  it('should write a .gitignore when none exists', () => {
    ...
    expect(ensureGitignore(dir, 'backend')).toBe(true)
...
describe('gitignore templates', () => {
  ...
  it('should ship a gitignore template for every project kind', () => {
    const templatesDir = getTemplatesDir()
    for (const kind of ['project', 'backend', 'frontend']) {
      expect(fs.existsSync(path.join(templatesDir, kind, 'gitignore'))).toBe(true)
    }
  })
})
